...
The source of truth is `manifest.json.version`. Tags prefix that value with `v`; the panel's displayed version must match. This first public line begins with `0.1.0-beta.1` and does not inherit any private development history.
