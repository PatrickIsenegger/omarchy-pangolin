...
Known limitations: English UI; configuration uses a JSON editor; only HTTP(S) web-app launchers; host ports/protocols sometimes require explicit overrides; Chromium is the web browser; resource lists refresh on panel open or manually; HTTPS diagnosis does not test UDP or prove app reachability.
